<template>
	<div class="flex justify-around flex-wrap text-[white] mx-auto my-0 lg:w-[calc(100%_-_40px)]">
		<div v-for="item in weather" :key="item.id"
			class="relative overflow-hidden bg-background-card flex flex-col justify-between w-[300px] cursor-default animate-[fadeIn_0.275s_ease-in-out] mb-5 p-5 rounded-[20px] ">
			...
			<div class="flex justify-center gap-2.5 transition-[0.275s]">
				<img :src="`https://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png`" alt="weather" />
				<h1>{{ Math.round(item.main.temp) }}°C</h1>
			</div>

			<p class="text-center m-1 p-0">Feels like {{ Math.round(item.main.feels_like) }}°C.</p>

			<p class="text-center m-1 p-0" v-for="weather in item.weather" :key="weather.id">
				{{ weather.description.charAt(0).toUpperCase() + weather.description.slice(1) }}.
			</p>

			<hr class="w-full h-px opacity-20 mt-5 rounded-[1px] border-[none] bg-background" />

			<div class="flex flex-wrap mt-2.5">
				<div class="w-[calc(50%_-_10px)] flex items-center mr-[5px] bg-background-shield shadow-[0px_0px_15px_#939090] mt-2.5 rounded-[5px]"
					title="Pressure">
					<svg class='w-5 h-5 fill-[white] my-[10px] mx-[5px]' viewBox="0 0 1000 1000">
						<g>
							<path
				...
				<div class="w-[calc(50%_-_10px)] flex items-center  bg-background-shield shadow-[0px_0px_15px_#939090] mt-2.5 rounded-[5px]"
					title="Wind speed">
					<svg class='w-5 h-5 fill-[white] my-[10px] mx-[5px]' viewBox="0 0 1000 1000">
						<g>
							<path
