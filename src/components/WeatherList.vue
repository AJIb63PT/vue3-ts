<template>
	<div class="weather_list">
		<div v-for="item in weather" :key="item.id" class="weather_list__card">
			<div class="weather_list__card__location_title">
				<p>{{ item.name }}</p>
				<img :srcset="`https://flagcdn.com/40x30/${item.sys?.country?.toLowerCase()}.png 2x, https://flagcdn.com/60x45/${item.sys.country.toLowerCase()}.png 3x`"
					width="20" height="15" :alt="item.sys?.country" />
			</div>

			<div class="weather_list__card__main_temperature">
				<img :src="`https://openweathermap.org/img/wn/${item.weather[0].icon}@2x.png`" alt="weather" />
				<h1>{{ Math.round(item.main.temp) }}°C</h1>
			</div>

			<p class="weather_list__card__description">Feels like {{ Math.round(item.main.feels_like) }}°C.</p>

			<p class="weather_list__card__description" v-for="weather in item.weather" :key="weather.id">
				{{ weather.description.charAt(0).toUpperCase() + weather.description.slice(1) }}.
			</p>

			<hr class="weather_list__card__divider" />

			<div class="weather_list__card__other">
				<div title="Pressure">
...
.weather_list {
	...
	&__card {
		...
		&__other {
			...
			flex-wrap: wrap;
			margin-top: 10px;
			text-shadow: 0px 0px 5px rgba(174, 134, 134, 0.25);
			...
			>div {
				...
				box-shadow: 0px 0px 15px #939090;
				border-radius: 5px;
...
.weather_list__card:hover .weather_list__card__background_image {
